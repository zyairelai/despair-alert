...
MONDAY_RANGE, WEEKLY_RANGE = True, True
    ...
    try:
        ...
        while True:
            ...
            if args.alert:
                ...
                alerts = []
                # Buffer 0.3% in the inner range (High - 0.3%, Low + 0.3%)
                if now_candle['high'] >= h1d * 0.997: alerts.append("Prev High")
                if now_candle['low'] <= l1d * 1.003: alerts.append("Prev Low")
                ...
                if alerts:
                    msg = f"{symbol_short} near/touch " + " & ".join(alerts)
